// argo:upstream 21st.dev@morphing-cursor — magnetic hover-text effect.
...
export function MagneticText({ text, hoverText = text, className }: MagneticTextProps) {
  ...
  return (
    ...
    >
      <span className="text-5xl font-bold tracking-tighter text-argo-text">{text}</span>
      <div
        ref={circleRef}
        className="absolute top-0 left-0 pointer-events-none rounded-full bg-argo-accent overflow-hidden"
        style={{
      ...
      >
        ...
        >
          <span className="text-5xl font-bold tracking-tighter text-argo-bg whitespace-nowrap">
            {hoverText}
          </span>
        </div>
      </div>
